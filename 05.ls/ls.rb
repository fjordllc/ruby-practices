...
begin
  if FileTest.directory?(ls_target_path)
    ...
    Dir.foreach(ls_target_path) do |item_in_dir|
      ...
      # print dir_items_list
    end
    # TODO: -rオプションの際にASC処理ではなくDESC処理をする
    dir_items_list = dir_items_list.sort
    max_vertical_items_count = (dir_items_list.size.to_f / HORIZONTAL_ITEMS_COUNT).ceil
    max_vertical_items_count.times do |count|
      dir_items_list.each_with_index do |v, i|
        next if i % max_vertical_items_count != count

        sorted_dir_items_list << v
      end
    end

    sorted_dir_items_list.each_slice(HORIZONTAL_ITEMS_COUNT) do |line_items|
